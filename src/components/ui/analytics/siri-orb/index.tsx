...
const SiriOrb: React.FC<SiriOrbProps> = ({
  ...
  return (
    <div
      className={cn("siri-orb", calibrating && "calibrating", className)}
      style={
        {
          width: size,
          height: size,
    ...
    >
      <style>{`
        ...
        .siri-orb {
          ...
          border-radius: 50%;
          position: relative;
          animation: ${breathing ? "breathing 4s ease-in-out infinite" : "none"};
          transition: all 0.5s ease;
        }

        .siri-orb.calibrating::before {
          animation: calibrate 1.2s cubic-bezier(0.16, 1, 0.3, 1) forwards;
        }
        ...
        @keyframes calibrate {
          0% {
            ...
            filter: blur(var(--blur-amount)) contrast(1.1) brightness(1.2);
          }
          33% {
            --c1: oklch(70% 0.12 142);
            --c2: oklch(75% 0.1 160);
            --c3: oklch(72% 0.12 150);
            filter: blur(calc(var(--blur-amount) * 1.5)) contrast(1.5) brightness(1.4);
          }
          66% {
            --c1: oklch(55% 0.25 35);
            --c2: oklch(65% 0.22 45);
            --c3: oklch(58% 0.24 40);
            filter: blur(calc(var(--blur-amount) * 0.8)) contrast(2) brightness(0.8);
          }
          100% {
            /* Return to current vars */
            filter: blur(var(--blur-amount)) contrast(var(--contrast-amount)) brightness(1);
          }
        }
